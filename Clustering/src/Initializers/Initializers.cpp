...
vector<vector<Point>*> Random_Selection<Point>::init(vector<vector<Point>>* dataset) {
    ...
    for (int i = 0; i < this->get_K(); i++) {
        centroids.push_back(&(*dataset)[distribution(generator)]);
    }
    return centroids;
}
